...
namespace Shuvaev.IDP.Controllers.Account
{
	...
	{
		private const string IdsrvFaSchemeName = "idsrv.2FA";
		...
		public async Task<IActionResult> Login(LoginInputModel model, string button)
		{
			...
			if (ModelState.IsValid)
			{
				...
				if (_userRepository.AreUserCredentialsValid(model.Username, model.Password))
				{
					...
					id.AddClaim(new Claim(JwtClaimTypes.Subject, user.SubjectId.ToString()));

					await HttpContext.SignInAsync(IdsrvFaSchemeName, new ClaimsPrincipal(id));
					// send code here with some provider (SMTP, SMS, etc.) ....
